			...
			try {
				...
				if (exists != null) {
					...
					if (rc.getSubtipo().contentEquals("OK")) {
						...
						this.proxy_os.writeObject(pd); // el proxy espera la ruta de la magen y se lo envia al
														// nodocentral
						this.client_os.writeObject(this.proxy_is.readObject()); // el proxy le manda al cliente lo que
																				// le dice el nodo central
				...
				try {
					if (this.client_is != null)
						this.client_is.close();
					if (this.client_os != null)
						this.client_os.close();
					if (this.proxy_is != null)
						this.proxy_is.close();
					if (this.proxy_os != null)
						this.proxy_os.close();
				} catch (IOException e) {
					e.printStackTrace();
				}
