...
public class NodoControl {
	...
	private static String NODES = "/home/agus/eclipse-workspace/TrabajoFinalSistB/src/main/resources/nodes.txt";
					...
					if (pd.getSubtipo().equals(lastPet.getSubtipo())) {
						...
						if (pd.getSubtipo().compareTo("OP_CPU") == 0) {
							for (int i = 0; i < pd.getCpus().size(); i++) {
								System.out.println(" > Nodo de token: " + pd.getTokens().get(i) + " - CPU: "
										+ pd.getCpus().get(i));
							}
